function [f,ff] = ChebTuck(fun,m,A,tol,r,type,method)
    ...
    % method: interpolation method, default is 'spline'. 'linear',
    % 'nearest', 'cubic' are also possible
    %
    ...
    % 
    % case 1, 2 corresponds to Alg. 3.1, 3.2 in the paper.
    % case 3 with 'R' (default) corresponds to Alg. 3.3 in the paper.
    %
    ...
    if isa(fun,'float')
        ...
        fun = @(x,y,z) interp3(xx,yy,zz,fun,x,y,z,method);
    ...
    elseif iscell(fun) && strcmp(type,'R')
        ...
        CU = {chebtech2.vals2coeffs(interp1(grids{1}',U{1},chebpts(m(1),[A(1,1),A(1,2)]),method,'extrap')), ...
            chebtech2.vals2coeffs(interp1(grids{2}',U{2},chebpts(m(2),[A(2,1),A(2,2)]),method,'extrap')), ...
            chebtech2.vals2coeffs(interp1(grids{3}',U{3},chebpts(m(3),[A(3,1),A(3,2)]),method,'extrap'))};
