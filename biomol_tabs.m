function biomol_tabs()
% parameters for ChebTuck
m = [129,129,129];
tol = 1e-7;

nlist = [256,512,1024,2048];
errors = zeros(4,2);
runtimes = zeros(4,2);

% load data
for ii = 1:4
    n1 = nlist(ii);Pn = 500;
    filename = ['data/Data_256-2048/n' num2str(n1) '_Pn' num2str(Pn) '.mat'];
    load(filename,"LRed","Hunif")
    h = 2/n1;
    xcol = -1+h/2:h:1-h/2;

    % get the CP tensor
    xi = LRed.LAM1C./Hunif^3;
    U = {LRed.CU1, LRed.CU2, LRed.CU3};
    % compute the middle slice of the tensor
    ns1 = floor(n1/2);
    X_slice = CP_get_subtensor(xi,U,{1:n1,1:n1,ns1});
    % uniform grid
    [xx, yy, zz] = ndgrid(xcol,xcol,xcol(ns1));

    % Alg 3.2
    if n1 == 256
        F = CP_get_subtensor(xi,U,{1:n1,1:n1,1:n1});
        tic
        f = ChebTuck(F,m,[],tol,[]);
        runtimes(ii,1) = toc;
        % compute the ChebTuck error
        X_ChebTuck = f(xx,yy,zz)';
        errors(ii,1) = norm(X_slice(:) - X_ChebTuck(:),'inf');
    end

    % Alg 3.3
    tic
    f = ChebTuck({xi,U},m,[],tol);
    runtimes(ii,2) = toc;
    % compute the ChebTuck error
    X_ChebTuck = f(xx,yy,zz);
    errors(ii,2) = norm(X_slice(:) - X_ChebTuck(:),'inf');
end
fprintf('\nTable 3: Error and runtime comparison\n');
fprintf('==========================================================================\n');
fprintf('          Alg. 3.2                            Alg. 3.3\n');
fprintf('--------------------------------------------------------------------------\n');
fprintf('  n       error         runtime            error              runtime\n');
fprintf('--------------------------------------------------------------------------\n');

for ii = 1:4
    fprintf('%5d', nlist(ii));
    if ii == 1
        fprintf('   %1.2e      %8.2f', errors(ii,1), runtimes(ii,1));
    else
        fprintf('      NA            NA   ');
    end
    fprintf('         %1.2e      %8.2f\n', errors(ii,2), runtimes(ii,2));
end
fprintf('==========================================================================\n');
end

